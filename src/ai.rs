...
fn minimax(
    ...
    let max_moves = if depth == config.max_depth { 15 } else { 10 };
    let moves_to_explore = moves.iter().take(max_moves);
    ...
    if maximizing {
        ...
        for &(x, y) in moves_to_explore {
            ...
            let eval = minimax(board, depth - 1, alpha, beta, false, player, config);
        ...
        for &(x, y) in moves_to_explore {
            board.set_cell(x, y, current_player).unwrap();
            let eval = minimax(board, depth - 1, alpha, beta, true, player, config);
            board.set_cell(x, y, Cell::Empty).unwrap();
...
pub fn find_best_move(board: &Board, player: Cell, config: &Config) -> Option<(usize, usize)> {
...
pub fn get_move(
    ...
    if let Some(move_pos) = find_best_move(board, player, config) {
        return Some(move_pos);
    }
